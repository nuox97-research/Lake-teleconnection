classdef corrMatPlot_hold_forCF
...
% CMP.setLabelStr({'welcome','to','follow','my','wechat','official','account','slandarer'})
    ...
    methods
        function obj=corrMatPlot_hold_forCF(varargin)
...
% =========================================================================
        function obj=draw(obj)
            ...
            % 绘制boxHdl
            if strcmp(obj.Type(end-1:end),'sq')
                vb0='off';
            else
                vb0='on';
            end
            ...
            maxcolortick = 2.4;
            dcolortick = 0.4;
            colorFunc=colorFuncFactory(obj.colorMap,mincolortick,maxcolortick);
            colormap(colorFunc(linspace(mincolortick,maxcolortick,1000)));
            caxis([mincolortick maxcolortick])
            cb = colorbar();
            % 绘制colorbar
            cb.Limits=[mincolortick maxcolortick];
            cb.TickDirection='out';
            cb.LineWidth=.8;
            cb.Ticks=mincolortick:dcolortick:maxcolortick;
            cb.FontName='Arial';
            cb.FontSize=25;
                    ...
                    switch obj.Type
                        ...
                        case 'ssq'   % 含文本方形
                            ...
                            obj.matrixHdl(row,col).t=text(col,1+obj.num-row,...
                                sprintf('%.2f',obj.corrMat(row,col)),'FontName','Arial','Fontweight','normal','FontSize',12,...
            ...
            for row=1:obj.num
                obj.RLabelHdl(row)=text(.42+(row-1).*strcmp(obj.Format,'triu'),obj.num+1-row,...
                    ...
                    'FontName','Arial','Fontweight','normal','FontSize',20);
            end
            for col=1:obj.num
                obj.CLabelHdl(col)=text(col-.1,(col-1).*strcmp(obj.Format,'tril')+.15,...
                    obj.Labels{col},'HorizontalAlignment','right',...
                    'FontName','Arial','Fontweight','normal','FontSize',20,'Rotation',45);
            end
            text()
            obj.drawn=true;
            ...
            % -------------------------------------------------------------
            % 渐变色句柄生成函数
            function colorFunc=colorFuncFactory(colorList,mincolortick,maxcolortick)
                x=linspace(mincolortick,maxcolortick,size(colorList,1));
...
% =========================================================================
        ...
        function obj=setColorMap(obj,CM)
            ...
            if obj.drawn
                ...
                colormap(colorFunc(linspace(0.1,0.3,100)));
                for row=1:obj.num
                for col=1:obj.num  
                    tValue=obj.corrMat(row,col);
                    tColor=colorFunc(tValue);
